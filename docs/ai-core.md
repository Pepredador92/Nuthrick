...
Resultados verificados: 578 tests frontend / 78 archivos en copia limpia de entrega; 15 pruebas Deno; TypeScript, Deno check, lint y build correctos. Los cambios no relacionados de LandingPage se conservaron sin incluir en la entrega (tienen un fallo de test propio en el árbol de trabajo). PostgreSQL serial y concurrencia pasaron. Edge `ai` v1 desplegada; acceso sin autenticar responde HTTP 401. Auditoría remota: cero features habilitadas, cero generaciones, ledger vacío, sin permisos monetarios para authenticated/anon y sin UPDATE/DELETE del ledger para service_role.
