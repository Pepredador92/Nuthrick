...
export function PublicProfilePage() {
  ...
  return <main className="min-h-screen bg-[#f7f8f4] text-[#17312c]">
    <header className="mx-auto flex max-w-6xl items-center justify-between px-5 py-6 sm:px-8"><Logo /><Link to="/register" className="text-sm font-semibold text-[#496e61]">¿Eres nutriólogo? Crea tu perfil</Link></header>
    <div className="mx-auto max-w-6xl px-5 pb-20 pt-8 sm:px-8">
      <section className="relative overflow-hidden rounded-[36px] bg-[#173d36] px-6 py-12 text-white sm:px-12 sm:py-16"><div className="absolute -right-20 -top-20 h-80 w-80 rounded-full border-[70px] border-white/5" /><div className="relative flex flex-col gap-8 md:flex-row md:items-center"><div className="grid h-32 w-32 shrink-0 place-items-center overflow-hidden rounded-[32px] bg-[#e4b272] text-4xl font-semibold text-[#17312c] shadow-xl">{profile.avatarUrl ? <img src={profile.avatarUrl} alt={`Foto de ${profile.name}`} className="h-full w-full object-cover" /> : profile.name.split(/\s+/).map((part) => part[0]).slice(0, 2).join('')}</div><div><p className="text-xs font-bold uppercase tracking-[.16em] text-[#efbd6b]">Perfil profesional</p><h1 className="mt-3 text-4xl font-semibold tracking-[-.04em] sm:text-5xl">{profile.name}</h1>{profile.professionalTitle && <p className="mt-3 text-lg text-white/65">{profile.professionalTitle}</p>}{profile.licenseNumber && <p className="mt-2 flex items-center gap-2 text-sm text-white/60"><BadgeCheck size={15} />Cédula profesional: {profile.licenseNumber}</p>}<div className="mt-5 flex flex-wrap gap-2">{modalities.map((item) => <span key={item} className="rounded-full bg-white/10 px-3 py-1.5 text-xs">{item}</span>)}{profile.country && <span className="inline-flex items-center gap-1.5 rounded-full bg-white/10 px-3 py-1.5 text-xs"><MapPin size={12} />{profile.country}</span>}</div></div></div></section>
      <div className="mt-6 grid gap-6 lg:grid-cols-[1.2fr_.8fr]">
        <div className="space-y-6">
          ...
          {contacts.length > 0 && <section className="nuth-public-card"><p className="nuth-eyebrow">Contacto</p><h2 className="mt-2 text-xl font-semibold">Conecta con {profile.name.split(' ')[0]}</h2><div className="mt-5 grid gap-3 sm:grid-cols-2">{contacts.map((contact) => contact.type === 'phone' ? <a key={`${contact.type}-${contact.value}`} href={whatsappUrl(contact)} target="_blank" rel="noreferrer" className="flex items-center gap-3 rounded-2xl bg-[#25D366] px-4 py-3.5 text-sm font-semibold text-white shadow-sm transition hover:bg-[#1fb857]"><MessageCircle size={21} /><span>WhatsApp{contact.label ? ` · ${contact.label}` : ''}</span></a> : <a key={`${contact.type}-${contact.value}`} href={`mailto:${encodeURIComponent(contact.value)}`} className="flex items-center gap-3 rounded-2xl bg-[#edf4ef] px-4 py-3.5 text-sm font-semibold text-[#356454] transition hover:bg-[#e1eee5]"><Mail size={20} /><span>{contact.label || 'Correo electrónico'}</span></a>)}</div></section>}
