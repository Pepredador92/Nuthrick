...
import { getSnapshot, listAnswers } from "@/src/services/consultations";
import type {
  Consultation,
  NutritionPlan,
  Patient,
...
function SnapshotHistory({ consultation }: { consultation: Consultation }) {
  const [snapshot, setSnapshot] = useState<import("@/src/types/domain").ConsultationSnapshot | null>(null);
  const [answerMap, setAnswerMap] = useState<Record<string, unknown>>({});
  useEffect(() => { let active = true; void Promise.all([getSnapshot(consultation.id), listAnswers(consultation.id)]).then(([loaded, answers]) => { if (active) { setSnapshot(loaded); setAnswerMap(Object.fromEntries(answers.map((answer) => [answer.question_key, answer.value]))); } }).catch(() => { if (active) setSnapshot(null); }); return () => { active = false; }; }, [consultation.id]);
  if (!snapshot) return null;
  return <section className="mt-5 rounded-2xl border border-[#dfe5e1] bg-white p-5"><p className="text-xs font-bold uppercase tracking-wide text-[#82908a]">{snapshot.template_name} · v{snapshot.template_version}</p><div className="mt-4 space-y-3">{snapshot.structure.sections.map((section, index) => <details key={section.section_key} open={index === 0} className="rounded-xl bg-[#f5f7f3] p-3"><summary className="cursor-pointer font-semibold text-[#315e4f]">{section.title}</summary><dl className="mt-3 space-y-3 text-sm">{section.questions.filter((question) => answerMap[question.question_key] !== undefined).map((question) => <div key={question.question_key} className="grid gap-1 sm:grid-cols-[minmax(0,190px)_1fr]"><dt className="text-[#74817d]">{question.label}</dt><dd className="whitespace-pre-wrap break-words text-[#42564d]">{typeof answerMap[question.question_key] === "string" ? answerMap[question.question_key] as string : JSON.stringify(answerMap[question.question_key])}</dd></div>)}</dl></details>)}</div></section>;
}
...
export function PatientDetailPage({ recordMode = false }: { recordMode?: boolean }) {
  ...
  const addConsultation = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); navigate(`/app/patients/${patient.id}/consultations/new`); };
    ...
    <div>
      ...
      <header className="mt-6 rounded-[28px] bg-[#173d36] p-6 text-white sm:p-8">
        <div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
          ...
          <div className="flex flex-wrap gap-2"><button type="button" className="nuth-button !bg-[#efbd6b] !text-[#173d36]" onClick={() => navigate(`/app/patients/${patient.id}/consultations/new`)}><CalendarPlus size={16} />Nueva consulta</button><button type="button" className="rounded-xl border border-white/20 px-3 py-2 text-sm font-semibold" onClick={() => setEditing(true)}><Edit3 size={16} />Editar</button></div>
        </div>
