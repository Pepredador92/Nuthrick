...
function ConfirmDialog({
  ...
  const consultation = action === "consultation-delete";
  const title = note
    ? "¿Eliminar esta nota?"
    : consultation
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="patient-confirm-title"
    >
      <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl">
        <h2 id="patient-confirm-title" className="text-xl font-semibold">
          {title}
        </h2>
        ...
        <div className="mt-6 flex justify-end gap-3">
          ...
          <button
            type="button"
            disabled={busy}
            className={`nuth-button ${action !== "archive" ? "!bg-[#9b493a]" : ""}`}
            onClick={onConfirm}
          >
            {busy ? (
...
function ConsultationDetail({
  ...
  if (!consultation)
    return (
      <EmptyState
        ...
        description="Crea la primera consulta para comenzar el historial longitudinal."
      />
    );
  ...
  return (
    <div className="space-y-5">
      <div className="rounded-2xl border border-[#dfe5e1] bg-[#fbfcfa] p-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <button
              type="button"
              className="rounded-xl px-3 py-2 text-xs font-semibold text-[#9b493a] hover:bg-[#fbe9e5]"
        ...
        {consultation.summary && (
          <p className="mt-3 whitespace-pre-wrap text-sm leading-6 text-[#5f6f68]">
            {consultation.summary}
          </p>
        )}
        <div className="mt-4 flex flex-wrap gap-2 text-xs text-[#60726a]">
          {measurement && (
            <span className="rounded-full bg-white px-3 py-1.5">
              {measurement.weight_kg} kg · IMC {measurement.bmi}
            </span>
          )}
          {related.length > 0 && (
            <span className="rounded-full bg-white px-3 py-1.5">
      ...
      {related.map((submission) => {
        ...
        >((acc, response) => {
          (acc[response.section_key] ??= []).push(response);
          return acc;
        }, {});
        return (
          <section
          ...
          >
            <h4 className="font-semibold">
              Cuestionario{" "}
              {submission.questionnaire_type === "initial"
                ? "de inicio"
                : "de seguimiento"}{" "}
            ...
            {Object.keys(grouped).length === 0 ? (
              <p className="mt-3 text-sm text-[#74817d]">
                No hay respuestas capturadas.
              </p>
            ) : (
              <div className="mt-4 space-y-2">
                {Object.entries(grouped).map(
                  ([section, sectionResponses], index) => (
                    <details
                      key={section}
                      open={index < 1}
                      className="rounded-xl bg-[#f5f7f3] p-3"
                    >
                      ...
                      <dl className="mt-3 space-y-2 text-sm">
                        {sectionResponses.map((response) => (
                          <div
                            key={response.id}
                            className="grid gap-1 sm:grid-cols-[minmax(0,180px)_1fr]"
                          >
                            <dt className="text-[#74817d]">
                              {response.question_key}
                            </dt>
                            <dd className="break-words text-[#42564d]">
...
function HistoryModal({
  ...
  const submitEdit = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (editingNote && noteDraft.trim()) {
      onEditNote({ ...editingNote, content: noteDraft.trim() });
      setEditingNote(null);
      setNoteDraft("");
    }
  };
  return (
    ...
    >
      <div className="flex h-[100dvh] w-full flex-col overflow-hidden bg-white sm:h-[90vh] sm:max-w-6xl sm:rounded-[28px] sm:shadow-2xl">
        <header className="flex items-center justify-between gap-4 border-b border-[#e3eae4] px-5 py-4 sm:px-7">
          <div>
            <p className="nuth-eyebrow">Historial longitudinal</p>
        ...
        <nav className="flex gap-1 overflow-x-auto border-b border-[#e3eae4] px-5 sm:px-7">
          {(
            ...
            <button
              ...
              key={key}
              onClick={() => onTab(key)}
              className={`whitespace-nowrap border-b-2 px-3 py-3 text-sm font-semibold ${tab === key ? "border-[#3d705d] text-[#285647]" : "border-transparent text-[#86918c]"}`}
            >
              {label}
            </button>
          ))}
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-5 sm:p-7">
            ...
            <div className="grid gap-5 lg:grid-cols-[250px_1fr]">
              <div>
                <div className="lg:hidden">
                  <label
                    className="text-xs font-semibold uppercase tracking-wide text-[#82908a]"
                    htmlFor="consultation-select"
                  >
                  ...
                  <select
                    ...
                    onChange={(event) =>
                      onSelectConsultation(event.target.value)
                    }
                  >
                ...
                <div className="hidden space-y-2 lg:block">
                  ...
                  {consultations.map((item) => (
                    <button
                      type="button"
                      key={item.id}
                      className={`w-full rounded-xl p-3 text-left ${selectedConsultation?.id === item.id ? "bg-[#eaf3ed]" : "hover:bg-[#f5f7f3]"}`}
                      onClick={() => onSelectConsultation(item.id)}
                    >
                      <span className="block text-sm font-semibold text-[#315e4f]">
                        {consultationLabel(item)}
                      </span>
          ...
          {tab === "notes" && (
            <div className="grid gap-5 lg:grid-cols-[.8fr_1.2fr]">
              <form
                className="rounded-2xl border border-[#dfe5e1] bg-[#fbfcfa] p-5"
                onSubmit={onCreateNote}
              >
                <p className="text-sm font-semibold">Crear nota libre</p>
                <p className="mt-1 text-xs leading-5 text-[#74817d]">
                  Puedes asociarla a una consulta o dejarla como nota general.
                </p>
                <Textarea
                  name="content"
                  className="mt-4"
                  ...
                  required
                />
                <select name="consultation_id" className="nuth-input mt-3">
                ...
                <button className="nuth-button mt-4">
                  <Plus size={16} />
                  Guardar nota
                </button>
              </form>
              <div className="space-y-3">
                {notes.length ? (
                  notes.map((note) => (
                    <article
                      key={note.id}
                      className="rounded-2xl border border-[#dfe5e1] bg-white p-4"
                    >
                      <p className="whitespace-pre-wrap text-sm leading-6 text-[#42564d]">
                        {note.content}
                      </p>
                      <p className="mt-3 text-xs text-[#82908a]">
                        {formatPatientDate(note.updated_at)}
                      ...
                      <div className="mt-3 flex gap-3">
                        ...
                        <button
                          type="button"
                          className="text-xs font-semibold text-[#9b493a]"
                  ...
                  <EmptyState
                    title="Aún no has creado notas."
                    description="Las notas libres aparecerán aquí."
                  />
                )}
                {editingNote && (
                  <form
                    ...
                    onSubmit={submitEdit}
                  >
                    <p className="text-sm font-semibold">Editar nota</p>
                    <Textarea
                      value={noteDraft}
...
function RecentConsultations({
  ...
  if (!consultations.length) {
    return (
      <EmptyState
        title="Aún no hay consultas registradas."
        description="Registra una nueva consulta para comenzar."
      />
    );
  }
  return (
...
export function PatientDetailPage({
  ...
  const deleteConsultation = async () => {
    ...
    const id = confirm.consultation.id;
    setConfirm(null);
    await run(async () => {
  ...
  const addNote = async (event: FormEvent<HTMLFormElement>) => {
    ...
    await run(async () => {
      const created = await createPatientNote(
        ...
        String(form.get("consultation_id") || "") || null,
      );
      setNotes((current) => [created, ...current]);
      event.currentTarget.reset();
    }, "Nota guardada.");
  };
  const editNote = async (note: PatientNote) => {
    await run(async () => {
      const updated = await updatePatientNote(note.id, note.content);
      setNotes((current) =>
  ...
  const confirmAction = async () => {
    if (!confirm) return;
    if (confirm.action === "note-delete") return deleteNote();
    if (confirm.action === "consultation-delete") return deleteConsultation();
      ...
      {editing && (
        ...
        >
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <Field label="Lada" name="edit-country">
              <select
                name="country_code"
                className="nuth-input"
            ...
            <Field label="Fecha de nacimiento" name="edit-birth">
              <Input
                name="birth_date"
                type="date"
