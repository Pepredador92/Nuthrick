...
export function OnboardingPage() {
  ...
  const [draft, setDraft, clearDraft] = usePersistentState(`nuthrick:${user?.id ?? 'anonymous'}:onboarding`, { name: profile?.full_name ?? user?.user_metadata.full_name ?? '', title: profile?.professional_title ?? 'Nutrióloga/o', country: profile?.country ?? 'México', timezone: profile?.timezone ?? 'America/Mexico_City' });
  const { name, title, country, timezone } = draft;
