...
export function ConsultationMeasurements({ consultation, patient }: { consultation: Consultation; patient: Patient }) {
  ...
  const load = useCallback(async () => {
    ...
    try { const [loaded, loadedCalculations, interpretationData] = await Promise.all([loadConsultationMeasurements(consultation), loadCalculationCatalog(), loadInterpretationData(consultation.id)]); setReferences(interpretationData.references); setSavedCalculations(interpretationData.saved); setHasSavedResults(interpretationData.saved.length > 0 || consultation.status !== "draft"); setPregnant(interpretationData.pregnant); setPregnancyFromInterview(interpretationData.pregnancyFromInterview); setEdited(false); setCatalog(loaded.catalog.filter((item) => item.category !== "laboratory")); setCalculationCatalog(loadedCalculations); setWorkspace(loaded.workspaceIds); setFollowup(loaded.followupIds); setHasFollowup(loaded.hasFollowup); setPreviousValues(loaded.previousValues); setValues(Object.fromEntries(loaded.values.map((item) => [item.measurement_type_id, displayValue(item.value)]))); }
  ...
  const interpretations = Object.fromEntries(visibleEvaluations.filter((e) => e.state === "calculated" && e.rawResult !== undefined).map((e) => [e.item.code, !edited && hasSavedResults ? savedCalculations.find((s) => s.calculation_code === e.item.code)?.interpretation_snapshot ?? null : interpretResult(e.item.code, e.rawResult!, e.item.definition.unit, currentContext, references, consultation.id)]));
