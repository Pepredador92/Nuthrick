...
export function ConsultationMeasurements({ consultation, patient }: { consultation: Consultation; patient: Patient }) {
  ...
  const load = useCallback(async () => {
    ...
    try { const loaded = await loadConsultationMeasurements(consultation); setCatalog(loaded.catalog.filter((item) => item.category !== "laboratory")); setWorkspace(loaded.workspaceIds); setFollowup(loaded.followupIds); setHasFollowup(loaded.hasFollowup); setPreviousValues(loaded.previousValues); setValues(Object.fromEntries(loaded.values.map((item) => [item.measurement_type_id, displayValue(item.value)]))); }
  ...
  const saveValues = async () => {
    ...
    try { const payload = Object.fromEntries(catalog.flatMap((measurement) => { const value = toPayloadValue(measurement, values[measurement.id]); return value === undefined ? [] : [[measurement.id, value]]; })); const saved = await saveConsultationMeasurements(consultation, payload); setValues(Object.fromEntries(saved.map((item) => [item.measurement_type_id, displayValue(item.value)]))); const proposedFollowup = saved.map((item) => item.measurement_type_id).filter((id) => workspace.includes(id)); if (!hasFollowup && proposedFollowup.length) { const savedFollowup = await savePatientMeasurementFollowup(patient.id, proposedFollowup); setFollowup(savedFollowup); setHasFollowup(true); setNotice("Mediciones guardadas. El seguimiento de este paciente se creó con las mediciones registradas hoy."); } else setNotice(saved.length ? "Mediciones guardadas." : "No hay mediciones registradas en esta consulta."); }
