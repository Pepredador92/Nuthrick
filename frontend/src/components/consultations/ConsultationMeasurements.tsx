...
export function ConsultationMeasurements({ consultation, patient }: { consultation: Consultation; patient: Patient }) {
  ...
  const visibleWorkspaceMeasurements = workspaceMeasurements.filter((item) => item.code !== "height");
  ...
  return <section className="min-w-0" aria-label="Mediciones de la consulta">
    ...
    <section className="mt-6"><h3 className="text-base font-semibold text-[#173d36]">Espacio de trabajo</h3>{visibleWorkspaceMeasurements.length ? <div className="mt-3 grid min-w-0 gap-3 sm:grid-cols-2 xl:grid-cols-3">{visibleWorkspaceMeasurements.map((item) => <MeasurementField key={item.id} measurement={item} value={values[item.id]} onChange={(value) => setValue(item.id, value)} />)}</div> : <p className="mt-3 rounded-xl border border-dashed border-[#cbd9ce] p-4 text-sm text-[#718176]">Tu espacio está vacío. Busca una medición para agregarla.</p>}</section>
