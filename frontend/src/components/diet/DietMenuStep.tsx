...
function RecipeAdjustPanel({ recipe, foods, pending, mealType, busy, onCancel, onUse, onSaveCopy }: {
  ...
  return <div className="rounded-2xl border border-[#bfd1c6] bg-[#f9fbf8] p-4 shadow-[0_18px_45px_rgba(23,61,54,.12)]">
    ...
    <div className="mt-4 space-y-2">{adjusted.items.map((item) => {
      const currentId = item.food_item_id ?? item.food_snapshot.id;
      const alternatives = foods.filter((food) => food.active && food.group_code === item.food_snapshot.group_code);
      return <div key={item.id} className="grid grid-cols-[minmax(0,1fr)_90px_auto] items-center gap-2 rounded-xl bg-white px-3 py-2"><label className="min-w-0"><span className="sr-only">Cambiar {item.food_snapshot.name}</span><select aria-label={`Alimento para ${item.food_snapshot.name}`} className="w-full truncate bg-transparent text-sm font-medium text-[#315449] outline-none" value={currentId} onChange={(event) => {
        const replacement = foods.find((food) => food.id === event.target.value && food.group_code === item.food_snapshot.group_code);
      ...
      }}>{!alternatives.some((food) => food.id === currentId) && <option value={currentId}>{item.food_snapshot.name}</option>}{alternatives.map((food) => <option key={food.id} value={food.id}>{food.name}</option>)}</select><span className="mt-0.5 block text-[11px] font-normal text-[#78867f]">{format(item.exchange_contribution[0]?.portions ?? 0)} {getExchangeGroup(item.food_snapshot.group_code).shortName}</span></label><input aria-label={`Cantidad de ${item.food_snapshot.name}`} type="number" min="0.001" step="0.5" className="nuth-input !py-2" value={amounts[item.id]} onChange={(event) => setAmounts({ ...amounts, [item.id]: Number(event.target.value) })} /><span className="text-xs text-[#718078]">{unitLabels[item.unit]}</span></div>;
    })}</div>
...
function MenuProposalPanel({ proposal, mode, canReplace, onMode, onApply, onDiscard, onCreateRecipe }: {
  ...
  return <div className="rounded-2xl border border-[#bfd1c6] bg-[#f9fbf8] p-4 shadow-[0_18px_45px_rgba(23,61,54,.12)]">
    <div className="flex items-start justify-between gap-3"><div><p className="text-[11px] font-bold uppercase tracking-[.12em] text-[#477363]">Vista previa</p><h3 className="mt-1 font-semibold text-[#24463b]">Propuesta lista</h3><p className="mt-1 text-xs text-[#718078]">{complete} tiempos completos{pending ? ` · ${pending} con pendiente` : ""}. Nada se guarda hasta que la apliques.</p></div><button type="button" aria-label="Descartar propuesta" onClick={onDiscard}><X size={19} /></button></div>
    ...
    <div className="mt-4 grid gap-3 md:grid-cols-2">{proposal.meals.map((meal) => { const foods = meal.addedEntries.filter((entry) => entry.type === "food"); return <section key={meal.mealTimeId} className="rounded-xl border border-[#dce7df] bg-white p-3"><div className="flex items-center justify-between gap-2"><h4 className="text-sm font-semibold text-[#315449]">{meal.mealName}</h4><span className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${meal.complete ? "bg-[#e7f3e9] text-[#35624e]" : "bg-[#fff3dc] text-[#8a692d]"}`}>{meal.complete ? "Completo" : "Propuesta parcial"}</span></div><div className="mt-3 space-y-2">{meal.addedEntries.map((entry) => <div key={entry.id} className="rounded-lg bg-[#f5f8f5] px-3 py-2"><p className="text-sm font-medium text-[#315449]">{entry.name_snapshot}</p><p className="mt-0.5 text-[11px] text-[#718078]">{entry.type === "recipe" ? "Receta ajustable" : `${format(entry.quantity)} ${unitLabels[entry.unit]}`}</p></div>)}{!meal.addedEntries.length && <p className="text-xs text-[#718078]">No necesita cambios.</p>}</div>{foods.length >= 2 && <button type="button" className="mt-3 text-xs font-semibold text-[#3d705d]" onClick={() => onCreateRecipe(meal)}><Plus size={13} className="mr-1 inline" />Crear receta</button>}{meal.pending.length > 0 && <p className="mt-3 text-xs text-[#8a692d]">Falta: {meal.pending.map((item) => `${format(item.portions)} ${getExchangeGroup(item.group_code).shortName}`).join(" · ")}</p>}{meal.excess.length > 0 && <p className="mt-2 text-xs text-[#a64a3d]">Excede: {meal.excess.map((item) => `${format(item.portions)} ${getExchangeGroup(item.group_code).shortName}`).join(" · ")}</p>}</section>; })}</div>
...
export function DietMenuStep({ plan, onSave, onDraftChange, onGoToMeals, catalog }: Props) {
  ...
  const [panel, setPanel] = useState<"food" | "recipe" | "recipe_adjust" | "new_food" | "new_recipe" | "proposal_recipe" | "proposal_recipe_use" | null>(null);
  ...
  return <section className="min-w-0 overflow-hidden rounded-[24px] border border-[#dfe6e1] bg-white p-4 sm:p-6">
    ...
    <div className="mt-5 grid min-w-0 gap-4 lg:grid-cols-[190px_minmax(0,1fr)_230px]">
      ...
      <main className="min-w-0 rounded-2xl border border-[#e0e7e2] p-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-xs font-bold uppercase tracking-[.12em] text-[#477363]">{meal.time || "Hora abierta"}</p><h2 className="mt-1 text-xl font-semibold text-[#24463b]">{meal.display_name}</h2></div><div className="flex flex-wrap gap-2"><button type="button" className="nuth-button-secondary !px-3 !py-2" onClick={() => buildProposal(meal.id)}><Sparkles size={15} /> Proponer</button><button type="button" aria-label="Agregar alimento" className="nuth-button-secondary !px-3 !py-2" onClick={() => open("food")}><Apple size={15} /> Alimento</button><button type="button" aria-label="Agregar receta" className="nuth-button-secondary !px-3 !py-2" onClick={() => open("recipe")}><ChefHat size={15} /> Receta</button></div></div>
        ...
        {proposal && <div className="mt-4"><MenuProposalPanel proposal={proposal} mode={proposalMode} canReplace={Boolean(proposal.mealTimeId && entries.length)} onMode={(mode) => buildProposal(proposal.mealTimeId, mode)} onApply={() => { change(proposal.menu, true); setProposal(null); }} onDiscard={() => setProposal(null)} onCreateRecipe={startProposalRecipe} /></div>}
