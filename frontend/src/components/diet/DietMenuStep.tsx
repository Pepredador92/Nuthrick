...
function MenuProposalPanel({ proposal, mode, canReplace, foods, onMode, onApply, onDiscard, onCreateRecipe, onExchangeFood, onEditRecipe }: {
  ...
  return <div className="rounded-2xl border border-[#bfd1c6] bg-[#f9fbf8] p-4 shadow-[0_18px_45px_rgba(23,61,54,.12)]">
    ...
    <div className="mt-4 grid gap-3 md:grid-cols-2">{proposal.meals.map((meal) => { const individualFoods = meal.addedEntries.filter((entry) => entry.type === "food"); const coverage = [...meal.addedEntries.flatMap((entry) => entry.exchange_contributions.map((item) => item.group_code))]; return <section key={meal.mealTimeId} className="rounded-xl border border-[#dce7df] bg-white p-3"><div className="flex items-center justify-between gap-2"><h4 className="text-sm font-semibold text-[#315449]">{meal.mealName}</h4><span className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${meal.complete ? "bg-[#e7f3e9] text-[#35624e]" : "bg-[#fff3dc] text-[#8a692d]"}`}>{meal.complete ? "Completo" : "Propuesta parcial"}</span></div><div className="mt-3 divide-y divide-[#edf1ee]">{meal.addedEntries.map((entry) => <div key={entry.id} className="flex items-center justify-between gap-3 py-2.5">{entry.type === "food" && entry.food_snapshot ? <FoodExchangeSelector className="flex-1" food={{ id: entry.source_id, name: entry.name_snapshot, group_code: entry.food_snapshot.group_code }} foods={foods} onSelect={(replacement) => onExchangeFood(entry.id, replacement)} /> : <button type="button" className="min-w-0 flex-1 text-left text-sm font-semibold text-[#315449] hover:text-[#173d36]" onClick={() => onEditRecipe(entry, meal)}>{entry.name_snapshot}<span className="ml-1 inline text-xs font-normal text-[#718078]">Editar receta</span></button>}<div className="shrink-0 text-right"><p className="text-sm text-[#52675e]">{entry.type === "recipe" ? `${format(entry.quantity)} porción` : `${format(entry.quantity)} ${unitLabels[entry.unit]}`}</p><p className="mt-0.5 text-[11px] text-[#78867f]">{entry.exchange_contributions.map((item) => `${getExchangeGroup(item.group_code).shortName} · ${format(item.portions)} eq`).join(" · ")}</p></div></div>)}{!meal.addedEntries.length && <p className="py-2 text-xs text-[#718078]">No necesita cambios.</p>}</div>{individualFoods.length >= 2 && <button type="button" className="mt-3 text-xs font-semibold text-[#3d705d]" onClick={() => onCreateRecipe(meal)}><Plus size={13} className="mr-1 inline" />Crear receta</button>}<div className="mt-3 border-t border-[#edf1ee] pt-3"><p className="text-[10px] font-bold uppercase tracking-[.1em] text-[#718078]">Cobertura</p><div className="mt-2 flex flex-wrap gap-1">{[...new Set(coverage)].map((code) => <span key={code} className="rounded-full bg-[#edf5ef] px-2 py-0.5 text-[10px] font-semibold text-[#35624e]">✓ {getExchangeGroup(code).shortName}</span>)}</div>{meal.pending.length > 0 && <p className="mt-2 text-xs text-[#8a692d]">Falta: {meal.pending.map((item) => `${format(item.portions)} ${getExchangeGroup(item.group_code).shortName}`).join(" · ")}</p>}{meal.excess.length > 0 && <p className="mt-2 text-xs text-[#a64a3d]">Excede: {meal.excess.map((item) => `${format(item.portions)} ${getExchangeGroup(item.group_code).shortName}`).join(" · ")}</p>}</div></section>; })}</div>
