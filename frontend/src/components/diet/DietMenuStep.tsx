...
import { BrowserPanel, CustomFoodForm, FoodExchangeSelector, MealNeeds, RecipeAdjustPanel, RecipeForm, RecipeUsePreview } from "./MenuEditors";
...
type Panel = "food" | "recipe" | "drink" | "recipe_preview" | "adjust" | "new_food" | "new_recipe" | "selection" | "preferences" | null;
...
function MenuWorkspace({ plan, headerActions, onSave, onDraftChange, onGoToMeals, catalog, recipeWriter = createCustomRecipe }: Props) {
  ...
  const hasRecipe = entries.some(entry => entry.type === "recipe");
  const visibleNeeds = hasRecipe ? rows.filter(row => row.state !== "complete") : rows;
  ...
  return <section className="min-w-0 rounded-[28px] border border-[#dce6de] bg-[#fbfcf8] p-4 text-[#173d36] sm:p-6">
    ...
    {panel&&<EditorDialog title={isPantry?"Despensa":panel==="preferences"?"Preferencias explícitas":panel==="selection"?"Selecciona qué forma la preparación":"Preparación"} onClose={closePanel}>
      ...
      {panel==="recipe_preview"&&selectedRecipe&&<RecipeUsePreview recipe={selectedRecipe} menu={working} distribution={distribution} mealId={meal.id} pending={remaining} outcomeRows={calculateMenuStatus(addRecipeToMenu(working,distribution,meal.id,selectedRecipe),distribution).rows.filter(row=>row.meal_time_id===meal.id)} mealType={meal.meal_type} busy={busy} onCancel={()=>setPanel("recipe")} onUse={incorporateRecipe}/>}
