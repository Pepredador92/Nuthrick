...
export function DietMenuStep({ plan, onSave, onDraftChange, onGoToMeals, catalog }: Props) {
  ...
  return <section className="min-w-0 overflow-hidden rounded-[24px] border border-[#dfe6e1] bg-white p-4 sm:p-6">
    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><p className="nuth-eyebrow">Paso 5</p><h1 className="mt-2 text-2xl font-semibold text-[#173d36]">Construcción del menú</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-[#718078]">Representa los equivalentes de cada tiempo con alimentos y recetas. No se modifica la prescripción.</p></div><span className={`w-fit rounded-full px-3 py-1 text-xs font-semibold ${draft.status === "ready" ? "bg-[#e6f2e8] text-[#35624e]" : "bg-[#f5efe1] text-[#79643b]"}`}>{draft.status === "ready" ? "Listo" : entries.length ? "En edición" : "Sin iniciar"}</span></div>
    ...
    <div className="mt-5 grid min-w-0 gap-4 lg:grid-cols-[190px_minmax(0,1fr)_230px]">
      ...
      <aside className="min-w-0 rounded-2xl border border-[#e0e7e2] p-4"><div className="flex items-center gap-2"><AlertTriangle size={16} className="text-[#b68437]" /><h2 className="font-semibold text-[#315449]">Equivalentes del tiempo</h2></div><p className="mt-1 text-xs leading-5 text-[#718078]">Pulsa un faltante para buscar dentro de ese grupo.</p><div className="mt-3"><MealNeeds rows={mealRows} onSelect={(code) => open("food", code)} /></div></aside>
    </div>
