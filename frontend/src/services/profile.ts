...
function unwrap<T>(result: { data: T | null; error: { message: string } | null }): T {
  ...
  if (result.data === null) throw new Error('No se encontraron los datos solicitados.');
  return result.data;
}

export async function fetchOwnProfile(userId: string): Promise<ProfessionalProfile | null> {
...
export async function ensureOwnProfile(userId: string): Promise<ProfessionalProfile> {
  const current = await fetchOwnProfile(userId);
  if (current) return current;
  const { error } = await supabase.from('professional_profiles').insert({ id: userId });
...
export async function loadProfileWorkspace(userId: string): Promise<ProfileWorkspace> {
  ...
  const [business, contacts, locations, education, links, images, conditions, populations, selectedConditions, selectedPopulations, availability, slots] = await Promise.all([
    ...
    supabase.from('conditions').select('id,slug,name').eq('is_active', true).order('name'),
    supabase.from('patient_populations').select('id,slug,name').eq('is_active', true).order('name'),
...
export async function addEducation(values: Omit<EducationRecord, 'id' | 'professional_id'>): Promise<EducationRecord> {
  const { data, error } = await supabase.from('professional_education').insert(values).select().single();
...
export async function deleteEducation(id: string): Promise<void> {
  const { error } = await supabase.from('professional_education').delete().eq('id', id);
  if (error) throw error;
}

export async function addLink(values: Omit<ProfessionalLink, 'id' | 'professional_id'>): Promise<ProfessionalLink> {
  const { data, error } = await supabase.from('professional_links').insert(values).select().single();
  return unwrap({ data: data as ProfessionalLink | null, error });
}

export async function updateLink(
  id: string,
  values: Partial<Pick<ProfessionalLink, 'link_type' | 'title' | 'url' | 'display_order' | 'is_active'>>,
...
export async function addServiceImage(values: Omit<ServiceImage, 'id' | 'professional_id'>): Promise<ServiceImage> {
  const { data, error } = await supabase.from('professional_service_images').insert(values).select().single();
  return unwrap({ data: data as ServiceImage | null, error });
}

export async function deleteServiceImage(id: string): Promise<void> {
  const { error } = await supabase.from('professional_service_images').delete().eq('id', id);
  if (error) throw error;
}

export async function replaceCatalogSelections(kind: 'conditions' | 'populations', ids: string[]): Promise<void> {
  const rpc = kind === 'conditions' ? 'set_professional_conditions' : 'set_professional_populations';
  const argument = kind === 'conditions' ? { condition_ids: ids } : { population_ids: ids };
  const { error } = await supabase.rpc(rpc, argument);
  if (error) throw error;
}

export async function saveAvailability(values: Omit<AvailabilitySettings, 'professional_id'>): Promise<AvailabilitySettings> {
  const { data, error } = await supabase.from('availability_settings').upsert(values).select().single();
  return unwrap({ data: data as AvailabilitySettings | null, error });
}

export async function addAvailabilitySlot(values: Omit<AvailabilitySlot, 'id' | 'professional_id'>): Promise<AvailabilitySlot> {
  const { data, error } = await supabase.from('availability_slots').insert(values).select().single();
...
export async function getPublicProfile(slug: string): Promise<PublicProfileContent | null> {
  const { data, error } = await supabase
  ...
  return {
    ...
    business: content.business ? { ...content.business, logoUrl: content.business.logoPath ? signed.get(content.business.logoPath) : undefined } : undefined,
    gallery: content.gallery.map((image) => ({ ...image, url: signed.get(image.path) })),
  };
}
