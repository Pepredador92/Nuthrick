...
export function formatPatientDate(value: string | null | undefined, fallback = '—'): string {
  if (!value) return fallback;
  const date = new Date(value);
  return Number.isNaN(date.getTime()) ? fallback : new Intl.DateTimeFormat('es-MX', { dateStyle: 'medium' }).format(date);
}

export function patientInitials(patient: Pick<Patient, 'full_name'>): string {
  return patient.full_name.split(/\s+/).filter(Boolean).slice(0, 2).map((part) => part[0]).join('').toUpperCase() || 'P';
}
