...
import { Clock3, Play, RefreshCw, ShieldAlert, Webhook } from "lucide-react";
      ...
      {data && <>
        ...
        <section className="operations-grid">
          ...
          <article className="admin-card">
            ...
            <p className="admin-note">El contacto verificado se administra en Correo transaccional real.</p>
          </article>
        </section>
        <EmailOperationsPanel />
        <section className="admin-card billing-section">
          <h2>Legal versionado</h2>
          <p className="admin-note">Abre cada documento para revisar el texto, sus decisiones pendientes y el historial de versiones.</p>
          <div className="operations-legal-list">{data.legal.documents.map((doc) => <span key={doc.key}><Link to={`/admin/legal/${doc.key}`}>{doc.title} · v{doc.version}</Link><strong className={doc.review_status === "approved" ? "operations-ok" : "operations-pending"}>{doc.review_status === "approved" ? "Aprobado" : "Pendiente de revisión"}</strong></span>)}</div>
        </section>
