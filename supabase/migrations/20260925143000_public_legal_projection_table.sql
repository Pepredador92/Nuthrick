-- Replace the public legal view with a small RLS-protected projection table.
do $$
begin
  if exists(select 1 from pg_class c join pg_namespace n on n.oid=c.relnamespace where n.nspname='public' and c.relname='legal_documents_public' and c.relkind='v') then
    execute 'drop view public.legal_documents_public';
  end if;
end $$;
...
language sql security invoker set search_path='' as $$
  select to_jsonb(d) from public.legal_documents_public d where d.key=p_document_key
$$;
