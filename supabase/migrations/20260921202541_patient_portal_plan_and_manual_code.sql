begin;
alter table private.patient_portals add column shared_plan_id uuid references public.nutrition_plans(id) on delete set null;
create index patient_portals_shared_plan on private.patient_portals(shared_plan_id) where shared_plan_id is not null;
alter table private.portal_challenges alter column email drop not null;
alter table private.portal_sessions alter column email drop not null;
alter table private.portal_challenges add column channel text not null default 'email' check(channel in ('email','professional'));
update private.patient_portals set recipient_email=null where recipient_email='';
grant select on public.nutrition_plans,public.nutrition_plan_versions to service_role;
...
begin
  ...
  if p_action in ('plan','plan_options','plan_preview','share_plan') then
