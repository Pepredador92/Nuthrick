...
try {
 ...
 if(process.argv.includes('--sql')) {
  ...
  if(updating && !requested) console.log(readFileSync(new URL('./library-estimation-guard.sql',import.meta.url),'utf8'));
  for(const item of items) {
   ...
   const key=quote(item.provenance.source_key);
   const match=`owner_id is null and provenance->>'kind'='provided_pdf' and (provenance->>'source_key'=${key} or split_part(provenance->>'label',' · ',2)=${key})`;
   const write=updating
    ? `update public.diet_library_items as library set name=${quote(item.name)},content=expanded.content,provenance=${quote(JSON.stringify(item.provenance))}::jsonb from expanded where ${match} and (provenance->>'import_version') is distinct from '2'`
    : `insert into public.diet_library_items(owner_id,name,content,provenance) select null,${quote(item.name)},content,${quote(JSON.stringify(item.provenance))}::jsonb from expanded where not exists(select 1 from public.diet_library_items where ${match})`;
