...
try {
  ...
  const payload=JSON.stringify({kind:'appointment',name:'Contacto sintético',start,modality:'online'});
  const book=(proof,operation)=>`select public.agenda_book('agenda-test','race-proof-${proof}','${operation}','${payload}');`;
  const session=(app)=>{
  ...
  assert.equal(run(database,"select count(*) from private.agenda_outbox where kind='confirmation';"),'1');
