...
const migration = readdirSync(new URL('../supabase/migrations/', import.meta.url)).filter(name=>name.endsWith('_patient_superlink.sql')||name.endsWith('_patient_portal_plan_and_manual_code.sql')).sort().map(name=>read(`../supabase/migrations/${name}`).replace(/^begin;\s*/m, '').replace(/commit;\s*$/, '')).join('\n');
